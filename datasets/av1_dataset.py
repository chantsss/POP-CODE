...
#
import osfrom itertools import permutationsfrom itertools import productfrom typing import Callable, Dict, List, Optional, Tuple, Union

import numpy as np
import pandas as pd
import torchfrom argoverse.map_representation.map_api import ArgoverseMapfrom torch_geometric.data import Datafrom torch_geometric.data import Datasetfrom tqdm import tqdmfrom aug.aug_base import AugBasefrom utils.utils_hivt import TemporalData
...
class Av1dataset(Dataset):
    ...
    def __init__(self,
        ...
        super(Av1dataset, self).__init__(root, transform=transform)
        if data_subset < 1.0:
            print(
                f"Using only {data_subset * 100}% of the data, {len(self._processed_paths)} files"
            )
            self._processed_paths = self._processed_paths[: int(len(self._processed_paths) * data_subset)]
            print(f"New number of files: {len(self._processed_paths)}")
    ...
    def processed_dir(self) -> str:
        if 'hivt' in self.model:
            return os.path.join(self.root, 'hivt', self._split, 'processed') 
