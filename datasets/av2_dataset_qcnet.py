...
import pickle
import shutil
import sysfrom pathlib import Pathfrom typing import Any, Callable, Dict, List, Mapping, Optional, Tuple, Unionfrom urllib import request

import numpy as np
import pandas as pd
import torchfrom torch_geometric.data import Datasetfrom torch_geometric.data import HeteroDatafrom torch_geometric.data import extract_tarfrom tqdm import tqdm
from utils import safe_list_indexfrom utils import side_to_directed_lineseg
...
class ArgoverseV2DatasetQCNet(Dataset):
    ...
    def __init__(self,
        ...
        processed_dir = os.path.join(root, 'qcnet', split, 'processed')
        ...
        self.split_processed_file_names = False
        if data_subset < 1.0:
            print(
                f"Using only {data_subset * 100}% of the data, {len(self._processed_file_names)} files"
            )
            self.split_processed_file_names = True
            # self.processed_paths = self.processed_paths[: int(len(self.processed_paths) * data_subset)]
            self._processed_file_names = self._processed_file_names[: int(len(self._processed_file_names) * data_subset)]
            
            print(f"New number of files: {len(self._processed_file_names)}")
        
        self._process()

        super(ArgoverseV2DatasetQCNet, self).__init__(root=root, transform=transform, pre_transform=None, pre_filter=None)
    ...
    def len(self) -> int:
        if self.split_processed_file_names:
    ...
    def _process(self) -> None:
        ...
        if (os.path.isdir(self.processed_dir) and len(self.processed_file_names) == self.len())\
              or self.split_processed_file_names:
            return
        ...
        self.process()
        print('------------Preprocess Done!------------', file=sys.stderr)
