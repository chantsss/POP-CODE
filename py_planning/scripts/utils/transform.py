...
class XYYawTransform():
  ...
  def get_future_state_from_cv_ackermann_model(self, v, pred_horizon=12, dt=0.5):
      
      x, y, yaw = self.get_list()
      state_list =[]
      for i in range(pred_horizon):
        # state = ackermann_model(state, v, yaws[i], L, dt)
        x_next = x + v * math.cos(yaw) * dt
        y_next = y + v * math.sin(yaw) * dt
        state_list.append([x_next, y_next, yaw])
        x = x_next
        y = y_next
