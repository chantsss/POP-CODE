...
import imp
import os
import picklefrom enum import unique, Enumfrom math import sin, cosfrom typing import Tuple, Dict, Optional, List, Any
import numpy as np
import logging
import matplotlib.pyplot as plt
from envs.commonroad.simulation.sumo_simulation import SumoSimulationfrom envs.commonroad.simulation.utility import agent_limited_length, agent_limited_width

# third party
import thirdparty.configfrom commonroad.common.file_reader import CommonRoadFileReaderfrom commonroad.common.solution import Solutionfrom commonroad.planning.planning_problem import PlanningProblemSet, PlanningProblemfrom commonroad.scenario.scenario import Scenariofrom commonroad.common.util import Intervalfrom commonroad.scenario.obstacle import ObstacleRolefrom commonroad.prediction.prediction import TrajectoryPredictionfrom commonroad.scenario.trajectory import Trajectory, Statefrom commonroad.planning.goal import GoalRegionfrom sumocr.interface.ego_vehicle import EgoVehiclefrom sumocr.maps.sumo_scenario import ScenarioWrapperfrom sumocr.sumo_config.default import DefaultConfigfrom sumocr.sumo_docker.interface.docker_interface import SumoInterfacefrom sumocr.visualization.video import create_videofrom simulation.simulations import load_sumo_configuration, check_trajectories, create_video_for_simulation

# planner & predicion modulefrom planners.planner import BasicPlannerfrom models.prediction.model import BasicPredictionModelfrom preprocessor.commonroad.vectorizer import CommonroadVectorizerfrom preprocessor.commonroad.graph import CommonroadGraphProcessorfrom preprocessor.commonroad.hivt import CommonroadHiVTProcessor
import commonroadfrom commonroad.scenario.obstacle import ObstacleTypefrom utils.geometry import get_box

# utilsfrom envs.config import get_root2folder
import envs.module_pathfrom envs.navigation_map import NavigationMapfrom utils.file_io import read_yaml_configfrom utils.transform import XYYawTransformfrom envs.commonroad.simulation.rviz import SimulationVisualizerfrom type_utils.agent import EgoAgent
import envs.commonroad.module_path
...
def simulate_with_prediction(conf: DefaultConfig,
    ...
    ######################################################################
    config_file_path =\
        os.path.join(os.path.dirname(envs.commonroad.module_path.__file__), 
    ...
    if preprocess_mode == 'graph':
        ...
        is_graph_mode = True
        fextractor.simu = True
    elif preprocess_mode == 'vectorize':
        fextractor = CommonroadVectorizer(args=preprocess_config)
        fextractor.simu = True
    elif preprocess_mode == 'hivt':
        fextractor = CommonroadHiVTProcessor(args=preprocess_config)
        fextractor.simu = True
    else:
...
def simulate_with_planner(scenario_index: int,
    ...
    preprocess_config = read_yaml_config(os.path.join(
        os.path.dirname(envs.module_path.__file__), 
