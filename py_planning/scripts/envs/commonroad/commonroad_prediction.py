...
import thirdparty.configfrom commonroad.scenario.scenario import Scenario
from envs.commonroad.commonroad import CommonroadDataset
import type_utils.agent as agent_utils
import type_utils.state_trajectory as state_trajfrom utils.transform import XYYawTransformfrom commonroad.common.file_reader import CommonRoadFileReaderfrom commonroad.scenario.obstacle import ObstacleTypefrom envs.config import get_root2folderfrom preprocessor.utils import InterfaceMode
from preprocessor.commonroad.vectorizer import CommonroadVectorizerfrom preprocessor.commonroad.graph import CommonroadGraphProcessorfrom preprocessor.commonroad.hivt import CommonroadHiVTProcessorfrom models.common.utils import TemporalData
...
class CommonroadDatasetPredictionExtractor(CommonroadDataset):
  ...
  def read_data_from_file(self, idx: int, file_data: Dict):
    ...
    '''
    if type(file_data) == TemporalData:
      return file_data
  ...
  def extract_scenario_data(self, idx: int, scenario: Scenario) -> None:
    ...
    for frame_from in frame_from_list:
      ...
      for _, obstacle in enumerate(scenario.obstacles):
        ...
        if np.any(content['data']) == None: # target agent length is not enough
          print('Skip content data == None')
          del dict_data[(idx, agent_idx, frame_from)]
          continue
        ...
        if len(dict_data.keys()) >= self.file_data_num:
          ...
          filepath = os.path.join(self.save_path, self.get_filename(idx, agent_idx, frame_from))
          if self.process_mode == 'hivt':
            torch.save(TemporalData(**content['data']), filepath.split('.')[0]+'.pt')
          else:
            self.save_data(filepath, dict_data)
          dict_data.clear()
    ...
    if len(dict_data.keys()) > 0:
      filepath = os.path.join(self.save_path, self.get_filename(idx, -1, -1))
      if self.process_mode == 'hivt':
        torch.save(TemporalData(**content['data']), filepath.split('.')[0]+'.pt')
      else:
        self.save_data(filepath, dict_data)
