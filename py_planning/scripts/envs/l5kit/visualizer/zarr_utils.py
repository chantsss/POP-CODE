...
import numpy as npfrom envs.l5kit.config import NavMapConfigfrom envs.l5kit.map_api import MapAPI, InterpolationMethod, TLFacesColors

import thirdparty.configfrom l5kit.data import ChunkedDatasetfrom l5kit.data.filter import (filter_agents_by_frames, filter_agents_by_labels, filter_tl_faces_by_frames,
                               filter_tl_faces_by_status)from l5kit.data.labels import PERCEPTION_LABELSfrom l5kit.environment.envs.l5_env import EpisodeOutputGymfrom l5kit.geometry import transform_pointsfrom l5kit.rasterization.box_rasterizer import get_box_world_coords, get_ego_as_agentfrom l5kit.rasterization.semantic_rasterizer import indices_in_boundsfrom l5kit.sampling.agent_sampling import get_relative_posesfrom l5kit.simulation.unroll import SimulationOutput, UnrollInputOutputfrom l5kit.visualization.visualizer.common import (AgentVisualization, CWVisualization, EgoVisualization,
...
def _get_frame_data(mapAPI: MapAPI, frame: np.ndarray, agents_frame: np.ndarray,
    ...
    mid_lanes_vis: List[TrajectoryVisualization] = [] # the function added
    ...
    for idx, lane_idx in enumerate(lane_indices):
        ...
        mid_lane = lane_coords["xyz_midlane"][:, :2]
        left_lane = lane_coords["xyz_left"][:, :2]
        right_lane = lane_coords["xyz_right"][::-1, :2] # inverse of right lane
        # left_lane[0] is parallel to right_lane[-1]

        lanes_vis.append(LaneVisualization(xs=np.hstack((left_lane[:, 0], right_lane[:, 0])),
                                           ys=np.hstack((left_lane[:, 1], right_lane[:, 1])),
                                           color=lane_colour))
        mid_lanes_vis.append(TrajectoryVisualization(xs=mid_lane[:, 0], ys=mid_lane[:, 1],
                                                     color="black", legend_label="none", track_id=-2))
    ...
    return FrameVisualization(ego=ego_vis, agents=agents_vis, lanes=lanes_vis,
                              crosswalks=crosswalks_vis, trajectories=mid_lanes_vis)
...
def zarr_to_visualizer_scene(scene_dataset: ChunkedDataset, mapAPI: MapAPI,
    ...
    tls_frames = filter_tl_faces_by_frames(frames, scene_dataset.tl_faces) # traffic lights
...
def simulation_out_to_visualizer_scene(nav_map: navigation_map,
                                       sim_out: SimulationOutput, mapAPI: MapAPI) -> List[FrameVisualization]:
    ...
    len_actual_simu = len(ego_ins_outs)
    has_ego_info = len(ego_ins_outs) > 0
    has_agents_info = len(agents_ins_outs) > 0

    route_vis = []
    if nav_map.has_global_route(scene_id):
        routes = nav_map.scene_routing_result[scene_id][2]
        for groute in routes:
            route_vis.append(TrajectoryVisualization(xs=groute[:, 0], ys=groute[:, 1],
                                                     color="green", legend_label="global_route", track_id=-2))

    frames_vis: List[FrameVisualization] = []
    for frame_idx in range(len(frames)):
        if frame_idx >= len_actual_simu:
            break # quit when out of len_actual_simu
        ...
        # extract lanes, xxx to rviz
        frame_vis = _get_frame_data(mapAPI, frame, agents_frame, tls_frame)
        trajectories = frame_vis.trajectories # add more trajectories

        # >> extract global route
        for route in route_vis:
            trajectories.append(route)

        # >> extract ego trajs to rviz
        if has_ego_info:
        ...
        # >> extract other lines
        # by setting legend_label == "none", will not show the label.
        # TODO: add more line rviz
        # ego_ins_outs[frame_idx].outputs['your_key']

        # >> extract agent trajs to rviz
        # add a protection: and (len(agents_ins_outs) > frame_idx)
        if has_agents_info and (len(agents_ins_outs) > frame_idx):
