...
from collections import defaultdictfrom typing import Any, DefaultDict, Dict, List, Set

import bokeh.io
import bokeh.plotting
import numpy as npfrom bokeh.layouts import column, LayoutDOMfrom bokeh.models import CustomJS, HoverTool, Sliderfrom bokeh.plotting import ColumnDataSource

import thirdparty.configfrom l5kit.visualization.visualizer.common import (AgentVisualization, CWVisualization, EgoVisualization,
...
def visualize(scene_index: int, frames: List[FrameVisualization]) -> LayoutDOM:
    ...
    for trajectory_name in trajectories_labels:
        if trajectory_name == "none":
            # disable legend_label, as common lines
            f.multi_line(alpha=0.2, line_width=1, line_dash="dashed",
                         source=out[0][trajectory_name], color="color")
        else:
            # enable legend_label
