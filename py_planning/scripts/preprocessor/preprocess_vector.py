...
import os
from preprocessor.preprocessor import PreprocessInterfacefrom preprocessor.utils import AgentType, TrafficLightfrom utils.angle_operation import get_normalized_anglefrom utils.transform import XYYawTransform
...
class PreprocessVectorization(PreprocessInterface):
    ...
    def extract_target_agent_representation(self, idx: int, agent_idx: int) -> np.ndarray:
        ...
        if len(hist) < self.t_h_state_num and self.simu != True: # 过滤不够时长的数据
            # print('len(hist) < ', self.t_h_state_num)
            return None
