...
import torch.nn.functional as Ffrom natten import NeighborhoodAttention1Dfrom timm.models.layers import DropPath, trunc_normal_


class NatActorNet(nn.Module):
    def __init__(
        self,
        in_chans=3,
        embed_dim=32,
        ...
        self.embed = ConvTokenizer(in_chans, embed_dim)  # 32
        self.num_levels = len(depths)
        self.num_features = [int(embed_dim * 2**i) for i in range(self.num_levels)]

        if learnable_pos_encoding:
            assert seq_len is not None
            self.pos_embed = nn.Parameter(torch.Tensor(seq_len, self.num_features[0]))
        ...
        if self.gru:
            self.aggregator = nn.GRU(n, n, batch_first=True)
    ...
    def forward(self, x):
        ...
        if self.gru:
            out = self.aggregator(out.permute(0, 2, 1))[1].squeeze(0)
            return out
