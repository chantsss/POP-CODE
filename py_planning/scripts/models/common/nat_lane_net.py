...
class NatLaneNet(nn.Module):
    def __init__(
        ...
        self.use_proj = proj_dim is not None
        ...
        if proj_dim is not None:
            self.proj = nn.Linear(embed_dim * len(depths), proj_dim)
    ...
    def forward(self, x):
        ...
        out = torch.stack([x.permute(0, 2, 1).max(1).values for x in laterals], dim=1)
