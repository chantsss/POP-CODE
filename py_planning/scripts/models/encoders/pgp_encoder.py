...
import torch
import torch.nn as nnfrom torch.nn.utils.rnn import pack_padded_sequencefrom typing import Dict
...
class PGPEncoder(PredictionEncoder):
    ...
    def forward(self, inputs: Dict) -> Dict:
        ...
        target_agent_feats, target_agent_representation_mask = \
            drop_his_pgp(valid_observation_length=5, historical_steps=5, \
                                          reduce_his_length=True, random_his_length=True, 
            random_interpolate_zeros=True, target_agent_representation=target_agent_feats, \
                drop_all=False, drop_for_recons=False)
...
def drop_his_pgp(valid_observation_length, historical_steps, reduce_his_length, random_his_length, 
            random_interpolate_zeros, target_agent_representation, drop_all=False, drop_for_recons=False):

    target_agent_representation_mask = torch.zeros(target_agent_representation.shape[:2],\
                                                         device=target_agent_representation.device).bool()
    if reduce_his_length: 
        ...
        if random_interpolate_zeros:
            ...
            indices = torch.arange(2, historical_steps)
        ...
        if len(set_zeros) != 0:
            # batch_size = len(target_agent_representation)
            # print('Start forgetting history in training, remain ', valid_observation_length,' steps.......')
            # device = data['x'].device
            padding_mask_origin = target_agent_representation_mask
            # for bz_idx in range(batch_size):
            target_agent_representation[:, set_zeros] = 0
            target_agent_representation_mask[:, set_zeros] = 1

            target_agent_representation_mask = ~padding_mask_origin & target_agent_representation_mask


    return target_agent_representation, target_agent_representation_mask