...
import copy
import abc
from planners.st_search.zone_stv_search import ZoneStvGraphSearchfrom planners.interaction_space import InteractionFormat, InteractionSpacefrom type_utils.agent import EgoAgent
  ...
  def _edge_is_valid(self, range_iinfo_dict: Dict, parent_node: np.ndarray, child_nodes: np.ndarray) -> np.ndarray:
    ...
    if (range_iinfo_dict['has_interaction']) and (child_nodes.shape[0] > 0):
      ...
      for ii, child_edge_stva in enumerate(check_stva_array):    
        ...
        st_conditions[stop_locs, :, 1] = 1e+3 # those stop nodes should avoid collisions at whole future time
