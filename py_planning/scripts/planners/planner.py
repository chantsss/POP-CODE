import abcfrom typing import Dict, List, Tuple
import copy
import numpy as np
import time
import math
from envs.navigation_map import NavigationMapfrom envs.commonroad.simulation.utility import limit_agent_length_widthfrom utils.transform import XYYawTransformfrom type_utils.agent import EgoAgentfrom type_utils.trajectory_point import TrajectoryPointfrom planners.interaction_space import InteractionSpacefrom planners.lattice_planner.lattice_planner import LatticePlannerfrom pymath.curves.bspline import SplineCurvefrom shapely.geometry import Polygon
from planners.st_search.zone_stv_search import ZoneStvGraphSearchfrom planners.st_search.zone_stv_search_ca import ZoneStvGraphSearchCollisionAvoidancefrom planners.st_search.zone_stv_search_izone_ca import ZoneStvGraphSearchIZoneCollisionAvoidancefrom planners.st_search.zone_stv_search_conti import ZoneStvGraphSearchContingency
      ...
      for pi, path_dict in enumerate(candidate_paths):
        ...
        if self.planner_type == 'none_ca':
          ...
          st_searcher = ZoneStvGraphSearchIZoneCollisionAvoidance(
            ...
            s_end_need_stop=end_need_stop, path_s_interval=path_s_interval,
            enable_debug_rviz=enable_debug_rviz)
        elif (self.planner_type == 'conti') or\
             (self.planner_type == 'conti+'):
          st_searcher = ZoneStvGraphSearchContingency(
            ego_agent, self.ispace, path_s_samples, path_xyyawcurs,
            start_sva=start_sva, search_horizon_T=self.plan_horizon_T,
            planning_dt=self.planning_dt, prediction_dt=self.prediction_dt,
            s_end_need_stop=end_need_stop, path_s_interval=path_s_interval,
            prediction_num=self.predict_traj_mode_num, 
