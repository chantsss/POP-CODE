import torch.optimfrom typing import Dict, Union
import torch
import numpy as npfrom models.common.utils import TemporalData
...
def convert_float_to_double(data: Union[Dict, torch.Tensor]):
    """
    Utility function to convert double tensors to float tensors in nested dictionary with Tensors
    """
    if type(data) is torch.Tensor and data.dtype == torch.float32:
        return data.double()
    elif type(data) is dict:
        for k, v in data.items():
            data[k] = convert_float_to_double(v)
        return data
    else:
        return data
