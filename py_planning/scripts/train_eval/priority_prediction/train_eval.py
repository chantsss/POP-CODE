import torch.optim
import torch.utils.data as torch_datafrom typing import Dict
import torch
import time
import math
import os
import train_eval.utils as train_eval_u
from train_eval.priority_prediction.dataset import PriorityDatasetfrom train_eval.priority_prediction.model import PriorityPredictionfrom metrics.bce_loss import BinaryCrossEntropyLoss
...
class TrainAndEvaluator:
    ...
    def __init__(self, cfg: Dict, checkpoint_path=None, just_weights=False, writer=None):
        ...
        """
        # Initialize datasets:
        train_set = PriorityDataset(version='train', args=cfg)
        val_set = PriorityDataset(version='eval', args=cfg)
        ...
        # Initialize model
        self.model = PriorityPrediction(cfg['network'])
        ...
        # Initialize losses
        self.losses = [BinaryCrossEntropyLoss({})]
        self.loss_weights = [1.0]

        # Initialize metrics
        self.train_metrics = self.losses
        self.val_metrics = self.losses
    ...
    def run_epoch(self, mode: str, dl: torch_data.DataLoader):
        ...
        for i, data in enumerate(dl):
            ...
            # Aggregate metrics
            minibatch_metrics, epoch_metrics =\
                self.aggregate_metrics(epoch_metrics, minibatch_time, predictions, data['ground_truth'], mode)
