import os
import sys
import argparsefrom typing import List, Dict
import randomfrom utils.colored_print import ColorPrinter
import torch

import thirdparty.configfrom envs.commonroad.simulation.utility import save_solutionfrom commonroad.common.file_writer import CommonRoadFileWriter, OverwriteExistingFilefrom commonroad.common.solution import VehicleType, VehicleModel, CostFunctionfrom commonroad.scenario.scenario import Tag

import envs.configfrom envs.config import get_root2folderfrom utils.random_seed import set_random_seedsfrom utils.file_io import extract_folder_file_list, write_dict2binfrom envs.commonroad.simulation.config import SCENE_DIRS, INVALID_CITY_LIST, INVALID_CITY_KEY, CITY_SCENE_MAX_NUMfrom envs.commonroad.simulation.simulation import simulate_with_plannerfrom planners.speed_profile_generator.generator import SpeedProfileGeneratorfrom models.prediction.model_factory import PredictionModelFactory
...
if __name__ == '__main__':  
  ...
  # pickout_intersections = [
  #   671, 673, 674, 677, 689, 696, 703, 708, 712, 719, 722, 724, 734, 739, 753, 765,
  #   779, 783, 844, 857, 898, 912, 1028, 1029, 1041, 1050, 1092, 1135, 1367, 1405, 1425,
  #   1492, 1777, 1784, 1787, 1796, 1798, 1799, 1801, 1820, 1881, 2083, 378, 496, 542, 646, 652,
  #   656, 667, 671, 672, 673, 674, 677, 681, 700, 706, 707, 708, 710, 718, 724, 726, 734, 737,
  #   739, 740, 743, 745, 753, 755, 760, 768, 779, 780, 783, 784, 793, 803, 805, 809, 810, 811, 
  #   815, 816, 827, 828, 831, 835, 836, 838, 840, 844, 867, 881, 886, 909, 930, 936, 1011, 1029, 
  #   1041, 1088, 1092, 1106, 1128, 1167, 1367, 1405, 1430, 1777, 1779, 1780, 1781, 1784, 1787, 1796,
  #   1799, 1801, 1820, 1822, 1881, 1933, 1945, 1980, 2083, 2257, 2468, 
  # ]

  # exclude_intersections = [
  #   706, 707, 712, 726, 734, 811, 898, 909, 378, 496
  # ]

  # pickout_intersections = [x for x in pickout_intersections if x not in exclude_intersections]
