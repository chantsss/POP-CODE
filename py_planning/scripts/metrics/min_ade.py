from metrics.metric import Metricfrom typing import Dict, Union
import torchfrom metrics.utils import min_ade
...
class MinADEK(Metric):
    ...
    def compute(self, predictions: Dict, ground_truth: Union[Dict, torch.Tensor]) -> torch.Tensor:
        ...
        """
        # Unpack arguments
        traj = predictions['traj']  # torch.Size([6, 24, 12, 4])
        probs = predictions['probs'] # torch.Size([24, 6])
        traj_gt = ground_truth['traj'] if type(ground_truth) == dict else ground_truth

        if traj.shape[0] != probs.shape[0]:
