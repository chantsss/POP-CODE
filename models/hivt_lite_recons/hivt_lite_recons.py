...
import copy
from utils.utils_hivt import TemporalDatafrom metrics import MR, berierFDE, minADE, minFDE, avgMinADE, avgMinFDEfrom metrics.analysis_metric import AnalysisMetricfrom submission.submission_base import SubmissionBase
from .decoder import GaussianDecoder, MLPDecoderfrom .global_interactor import GlobalInteractorfrom .local_encoder import LocalEncoderfrom .losses.gaussian_nll_loss import GaussianNLLLossfrom .losses.laplace_nll_loss import LaplaceNLLLossfrom .losses.multipath_loss import MultipathLaplaceLoss, MultipathLossfrom .losses.nll_cls_loss import NllClsLossfrom .losses.soft_target_cross_entropy_loss import SoftTargetCrossEntropyLossfrom models.common.utils import drop_hisfrom models.hivt_lite import HiVTLite

class HiVTLiteRcons(pl.LightningModule):
    def __init__(
        self,
        ...
        self.only_distill = only_distill
        ...
        if not self.only_train_recons:
            if self.use_cross_attention_by_layer and not self.only_distill:
    ...
    def forward(self, data: TemporalData):
        ...
        # Recons forward
        if not self.muliti_task and not self.only_distill:
        ...
        if self.only_train_recons:
            return y_hat_init, pi_init, y_hat_recons, pi_recons, y_hat_refine, pi_refine
        
        # Multi task forward
        if self.muliti_task:
            y_hat_recons, pi_recons = self.decoder_recons(local_embed=local_embed, global_embed=global_embed)
            if self.distillation:
                return y_hat_init, pi_init, y_hat_recons, pi_recons, \
                y_hat_refine, pi_refine, teacher_local_embed, \
                teacher_global_embed, local_embed, global_embed
            else:
                return y_hat_init, pi_init, y_hat_recons, \
        ...
        if self.distillation:
            return y_hat_init, pi_init, y_hat_recons, pi_recons, y_hat_refine, pi_refine, \
            teacher_local_embed, teacher_global_embed, local_embed_refine, global_embed_refine
