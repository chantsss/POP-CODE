...
import mathfrom typing import Dict, List, Mapping, Optional

import torch
import torch.nn as nn
import torch.nn.functional as Ffrom torch_cluster import radiusfrom torch_cluster import radius_graphfrom torch_geometric.data import Batchfrom torch_geometric.data import HeteroDatafrom torch_geometric.utils import dense_to_sparse
from layers import AttentionLayerfrom layers import FourierEmbeddingfrom layers import MLPLayerfrom utils import angle_between_2d_vectorsfrom utils import bipartite_dense_to_sparsefrom utils import weight_initfrom utils import wrap_angle
...
class QCNetDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 dropout: float,
                 recons: bool,
                 use_recons_cross_attention: bool,
                 use_recons_concat: bool,
                 num_modes_recons: int,) -> None:
        ...
        if self.recons:
            # recons emb
            self.mode_emb_recons = nn.Embedding(self.num_modes_recons, hidden_dim)
            self.r_t2m_emb_recons = FourierEmbedding(input_dim=input_dim_r_t, hidden_dim=hidden_dim, num_freq_bands=num_freq_bands)
            self.r_pl2m_emb_recons = FourierEmbedding(input_dim=input_dim_r_pl2m, hidden_dim=hidden_dim,
                                            num_freq_bands=num_freq_bands)
            self.r_a2m_emb_recons = FourierEmbedding(input_dim=input_dim_r_a2m, hidden_dim=hidden_dim,
                                            num_freq_bands=num_freq_bands)

            #recons attention
            self.t2m_propose_attn_layers_recons = nn.ModuleList(
                [AttentionLayer(hidden_dim=hidden_dim, num_heads=num_heads, head_dim=head_dim, dropout=dropout,
                                bipartite=True, has_pos_emb=True) for _ in range(num_layers)]
            )
            self.pl2m_propose_attn_layers_recons = nn.ModuleList(
                [AttentionLayer(hidden_dim=hidden_dim, num_heads=num_heads, head_dim=head_dim, dropout=dropout,
                                bipartite=True, has_pos_emb=True) for _ in range(num_layers)]
            )
            self.a2m_propose_attn_layers_recons = nn.ModuleList(
                [AttentionLayer(hidden_dim=hidden_dim, num_heads=num_heads, head_dim=head_dim, dropout=dropout,
                                bipartite=True, has_pos_emb=True) for _ in range(num_layers)]
            )

            if self.use_recons_cross_attention:
                self.f_h_cross_attention_module = torch.nn.MultiheadAttention(embed_dim = hidden_dim, num_heads = num_heads, dropout = dropout)

            self.m2m_propose_attn_layer_recons = AttentionLayer(hidden_dim=hidden_dim, num_heads=num_heads, head_dim=head_dim,
                                                        dropout=dropout, bipartite=False, has_pos_emb=False)
            self.to_loc_propose_pos_recons = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
                                            output_dim=self.num_future_steps_recons * output_dim // self.num_recurrent_steps_recons)
            self.to_scale_propose_pos_recons = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
                                                output_dim=self.num_future_steps_recons * output_dim // self.num_recurrent_steps_recons)
            # self.to_loc_refine_pos_recons = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
            #                                 output_dim=self.num_future_steps_recons * output_dim)
            # self.to_scale_refine_pos_recons = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
            #                                     output_dim=self.num_future_steps_recons * output_dim)
            if self.output_head:
                self.to_loc_propose_head_recons = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
                                                    output_dim=self.num_future_steps_recons // self.num_recurrent_steps_recons)
                self.to_conc_propose_head_recons = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
                                                    output_dim=self.num_future_steps_recons // self.num_recurrent_steps_recons)
                ...
                self.to_loc_propose_head_recons = None
                self.to_conc_propose_head_recons = None
        ...
        self.to_scale_propose_pos = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
                                             output_dim=num_future_steps * output_dim // num_recurrent_steps)
        if self.use_recons_cross_attention and slef.use_recons_concat:
            self.to_loc_refine_pos = MLPLayer(input_dim=2*hidden_dim, hidden_dim=hidden_dim,
                                            output_dim=num_future_steps * output_dim)
            self.to_scale_refine_pos = MLPLayer(input_dim=2*hidden_dim, hidden_dim=hidden_dim,
                                                output_dim=num_future_steps * output_dim)
        else:
        ...
        if output_head:
            ...
            self.to_conc_propose_head = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
                                                 output_dim=num_future_steps // num_recurrent_steps)
            if self.use_recons_cross_attention and self.use_recons_concat:
                self.to_loc_refine_head = MLPLayer(input_dim=2*hidden_dim, hidden_dim=hidden_dim, output_dim=num_future_steps)
                self.to_conc_refine_head = MLPLayer(input_dim=2*hidden_dim, hidden_dim=hidden_dim,
                                                    output_dim=num_future_steps)
            else:
                self.to_loc_refine_head = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=num_future_steps)
                self.to_conc_refine_head = MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
            ...
            self.to_conc_refine_head = None
        if self.use_recons_cross_attention and self.use_recons_concat:
            self.to_pi = MLPLayer(input_dim=2*hidden_dim, hidden_dim=hidden_dim, output_dim=1)
        else:
    ...
    def forward(self,
                ...
                recons: bool,) -> Dict[str, torch.Tensor]:
        ...
        m_propose = m.reshape(-1, self.num_future_steps, self.hidden_dim).transpose(0, 1)

        m_refine = self.traj_emb(m_propose, self.traj_emb_h0.unsqueeze(1).repeat(1, m_propose.size(1), 1))[1].squeeze(0)

        # # recons 
        # recons_temp = recons if self.recons != recons else self.recons
        # if recons_temp:
        #     reconstructions = self.reconstruct(data, scene_enc)
        #     m_recons = reconstructions['m_recons'].reshape(self.num_modes_recons, -1, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim)
        #     if self.use_recons_cross_attention:
        #         m_refine = m_refine.reshape(self.num_modes, -1, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim)
        #         m_refine, _ = self.f_h_cross_attention_module(m_refine, m_recons, m_recons)

        for i in range(self.num_layers):
            m_refine = self.t2m_refine_attn_layers[i]((x_t, m_refine), r_t2m, edge_index_t2m)
            m_refine = m_refine.reshape(-1, self.num_modes, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim)
            m_refine = self.pl2m_refine_attn_layers[i]((x_pl, m_refine), r_pl2m, edge_index_pl2m)
            m_refine = self.a2m_refine_attn_layers[i]((x_a, m_refine), r_a2m, edge_index_a2m)
            m_refine = m_refine.reshape(self.num_modes, -1, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim)
        m_refine = self.m2m_refine_attn_layer(m_refine, None, edge_index_m2m)
        ...
        if recons_temp:
            ...
            m_recons = reconstructions['m_recons'].reshape(self.num_modes_recons, -1, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim)
            if self.use_recons_cross_attention:
                if self.use_recons_concat:
                    m_refine_ = torch.concat((m_refine, m_recons.repeat(int(self.num_modes/self.num_modes_recons) ,1)), -1)
                    m_refine_ = m_refine_.reshape(-1, self.num_modes, 2*self.hidden_dim)
                else:
                    m_refine, _ = self.f_h_cross_attention_module(m_refine, m_recons, m_recons)
                    m_refine = m_refine.reshape(self.num_modes, -1, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim) 

        m_refine = m_refine.reshape(-1, self.num_modes, self.hidden_dim)

        if m_refine_ == None:
            m_refine_ = m_refine

        loc_refine_pos = self.to_loc_refine_pos(m_refine_).view(-1, self.num_modes, self.num_future_steps, self.output_dim)
        loc_refine_pos = loc_refine_pos + loc_propose_pos.detach()
        scale_refine_pos = F.elu_(
            self.to_scale_refine_pos(m_refine_).view(-1, self.num_modes, self.num_future_steps, self.output_dim),
            alpha=1.0) + 1.0 + 0.1
        if self.output_head:
            loc_refine_head = torch.tanh(self.to_loc_refine_head(m_refine_).unsqueeze(-1)) * math.pi
            loc_refine_head = loc_refine_head + loc_propose_head.detach()
            conc_refine_head = 1.0 / (F.elu_(self.to_conc_refine_head(m_refine_).unsqueeze(-1)) + 1.0 + 0.02)
        else:
            ...
            conc_refine_head = scale_refine_pos.new_zeros((scale_refine_pos.size(0), self.num_modes,
                                                           self.num_future_steps, 1))
        pi = self.to_pi(m_refine_).squeeze(-1)
        ...
        return {
            ...
            'pi': pi,
            'reconstructions': reconstructions if recons_temp else None,
            'm_propose': m_propose,
            'm_refine': m_refine,
        }
    ...
    def reconstruct(self,
        ...
        pos_m = data['agent']['position'][:, self.num_future_steps_recons - 1, :self.input_dim]
        head_m = data['agent']['heading'][:, self.num_future_steps_recons - 1]
        head_vector_m = torch.stack([head_m.cos(), head_m.sin()], dim=-1)

        x_t = scene_enc['x_a'].reshape(-1, self.hidden_dim)
        x_pl = scene_enc['x_pl'][:, self.num_future_steps_recons - 1].repeat(self.num_modes_recons, 1)
        x_a = scene_enc['x_a'][:, -1].repeat(self.num_modes_recons, 1)
        m = self.mode_emb_recons.weight.repeat(scene_enc['x_a'].size(0), 1)

        mask_src = data['agent']['valid_mask'][:, :self.num_future_steps_recons].contiguous()
        mask_src[:, :self.num_future_steps_recons - self.num_t2m_steps] = False
        mask_dst = data['agent']['predict_mask_his'].any(dim=-1, keepdim=True).repeat(1, self.num_modes_recons)

        pos_t = data['agent']['position'][:, :self.num_future_steps_recons, :self.input_dim].reshape(-1, self.input_dim)
        head_t = data['agent']['position'][:, :self.num_future_steps_recons].reshape(-1)
        edge_index_t2m = bipartite_dense_to_sparse(mask_src.unsqueeze(2) & mask_dst[:, -1:].unsqueeze(1))
        rel_pos_t2m = pos_t[edge_index_t2m[0]] - pos_m[edge_index_t2m[1]]
        rel_head_t2m = wrap_angle(head_t[edge_index_t2m[0]] - head_m[edge_index_t2m[1]])
        r_t2m = torch.stack(
            [torch.norm(rel_pos_t2m[:, :2], p=2, dim=-1),
             ...
             rel_head_t2m,
             (edge_index_t2m[0] % self.num_historical_steps) - self.num_historical_steps + 1], dim=-1)
        r_t2m = self.r_t2m_emb_recons(continuous_inputs=r_t2m, categorical_embs=None)
        edge_index_t2m = bipartite_dense_to_sparse(mask_src.unsqueeze(2) & mask_dst.unsqueeze(1))
        r_t2m = r_t2m.repeat_interleave(repeats=self.num_modes_recons, dim=0)
        ...
        r_pl2m = torch.stack(
            [torch.norm(rel_pos_pl2m[:, :2], p=2, dim=-1),
             angle_between_2d_vectors(ctr_vector=head_vector_m[edge_index_pl2m[1]], nbr_vector=rel_pos_pl2m[:, :2]),
             rel_orient_pl2m], dim=-1)
        r_pl2m = self.r_pl2m_emb_recons(continuous_inputs=r_pl2m, categorical_embs=None)
        edge_index_pl2m = torch.cat([edge_index_pl2m + i * edge_index_pl2m.new_tensor(
            [[data['map_polygon']['num_nodes']], [data['agent']['num_nodes']]]) for i in range(self.num_modes_recons)], dim=1)
        r_pl2m = r_pl2m.repeat(self.num_modes_recons, 1)
        ...
        r_a2m = torch.stack(
            [torch.norm(rel_pos_a2m[:, :2], p=2, dim=-1),
             angle_between_2d_vectors(ctr_vector=head_vector_m[edge_index_a2m[1]], nbr_vector=rel_pos_a2m[:, :2]),
             rel_head_a2m], dim=-1)
        r_a2m = self.r_a2m_emb_recons(continuous_inputs=r_a2m, categorical_embs=None)
        edge_index_a2m = torch.cat(
            [edge_index_a2m + i * edge_index_a2m.new_tensor([data['agent']['num_nodes']]) for i in
             range(self.num_modes_recons)], dim=1)
        r_a2m = r_a2m.repeat(self.num_modes_recons, 1)

        edge_index_m2m = dense_to_sparse(mask_dst.unsqueeze(2) & mask_dst.unsqueeze(1))[0]

        locs_propose_pos: List[Optional[torch.Tensor]] = [None] * self.num_recurrent_steps_recons
        scales_propose_pos: List[Optional[torch.Tensor]] = [None] * self.num_recurrent_steps_recons
        locs_propose_head: List[Optional[torch.Tensor]] = [None] * self.num_recurrent_steps_recons
        concs_propose_head: List[Optional[torch.Tensor]] = [None] * self.num_recurrent_steps_recons
        for t in range(self.num_recurrent_steps_recons):
            for i in range(self.num_layers):
                m = m.reshape(-1, self.hidden_dim)
                m = self.t2m_propose_attn_layers_recons[i]((x_t, m), r_t2m, edge_index_t2m)
                m = m.reshape(-1, self.num_modes_recons, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim)
                m = self.pl2m_propose_attn_layers_recons[i]((x_pl, m), r_pl2m, edge_index_pl2m)
                m = self.a2m_propose_attn_layers_recons[i]((x_a, m), r_a2m, edge_index_a2m)
                m = m.reshape(self.num_modes_recons, -1, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim)
            m = self.m2m_propose_attn_layer_recons(m, None, edge_index_m2m)
            m = m.reshape(-1, self.num_modes_recons, self.hidden_dim)
            locs_propose_pos[t] = self.to_loc_propose_pos_recons(m)
            scales_propose_pos[t] = self.to_scale_propose_pos_recons(m)
            if self.output_head:
                locs_propose_head[t] = self.to_loc_propose_head_recons(m)
                concs_propose_head[t] = self.to_conc_propose_head_recons(m)
        loc_propose_pos = torch.cumsum(
            torch.cat(locs_propose_pos, dim=-1).view(-1, self.num_modes_recons, self.num_historical_steps, self.output_dim),
            dim=-2)
        scale_propose_pos = torch.cumsum(
            F.elu_(
                torch.cat(scales_propose_pos, dim=-1).view(-1, self.num_modes_recons, self.num_historical_steps, self.output_dim),
                alpha=1.0) +
            1.0,
            dim=-2) + 0.1
        if self.output_head:
            ...
            loc_propose_head = loc_propose_pos.new_zeros((loc_propose_pos.size(0), self.num_modes_recons,
                                                          self.num_historical_steps, 1))
            conc_propose_head = scale_propose_pos.new_zeros((scale_propose_pos.size(0), self.num_modes_recons,
                                                             self.num_historical_steps, 1))
            # m = self.y_emb(loc_propose_pos.detach().view(-1, self.output_dim))
    ...
    def distill_forward(self,
                data: HeteroData,
                scene_enc: Mapping[str, torch.Tensor], distill_out: bool) -> Dict[str, torch.Tensor]:
        pos_m = data['agent']['position'][:, self.num_historical_steps - 1, :self.input_dim]
        head_m = data['agent']['heading'][:, self.num_historical_steps - 1]
        head_vector_m = torch.stack([head_m.cos(), head_m.sin()], dim=-1)
        ...
        if distill_out:
            loc_refine_pos = self.to_loc_refine_pos(m_refine).view(-1, self.num_modes, self.num_future_steps, self.output_dim)
            loc_refine_pos = loc_refine_pos + loc_propose_pos.detach()
            scale_refine_pos = F.elu_(
                self.to_scale_refine_pos(m_refine).view(-1, self.num_modes, self.num_future_steps, self.output_dim),
                alpha=1.0) + 1.0 + 0.1
            if self.output_head:
                loc_refine_head = torch.tanh(self.to_loc_refine_head(m_refine).unsqueeze(-1)) * math.pi
                loc_refine_head = loc_refine_head + loc_propose_head.detach()
                conc_refine_head = 1.0 / (F.elu_(self.to_conc_refine_head(m_refine).unsqueeze(-1)) + 1.0 + 0.02)
            else:
                loc_refine_head = loc_refine_pos.new_zeros((loc_refine_pos.size(0), self.num_modes, self.num_future_steps,
                                                            1))
                conc_refine_head = scale_refine_pos.new_zeros((scale_refine_pos.size(0), self.num_modes,
                                                            self.num_future_steps, 1))
            pi = self.to_pi(m_refine).squeeze(-1)
