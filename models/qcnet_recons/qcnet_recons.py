...
class QCNetRecons(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 **kwargs) -> None:
        super(QCNetRecons, self).__init__()
        ...
        self.decoder = QCNetDecoder(
            ...
            num_historical_steps=num_historical_steps,
            num_future_steps=num_historical_steps, # future == his
            num_modes=num_modes,
        ...
        if reduce_his_length: 
            ...
            if len(set_zeros) != 0:
                ...
                if not self.drop_all_agent:
                    ...
                    data['agent']['predict_mask'][..., :self.num_historical_steps] = data['agent']['valid_mask'][..., :self.num_historical_steps]
                    data['agent']['predict_mask'][..., self.num_historical_steps:] = data['agent']['predict_mask'][..., self.num_historical_steps:].fill_(False)
                    ...
                    data['agent']['valid_mask'][:, 1: historical_steps] = (
                            ...
                            data['agent']['valid_mask'][:, 1: historical_steps])
                    data['agent']['valid_mask'][:, 0] = data['agent']['valid_mask'][:, 0].fill_(False)
    ...
    def forward(self, data: HeteroData):
        ...
        return pred
# (Pdb) scene_enc['x_pt'].shape
# torch.Size([1078, 128])
# (Pdb) scene_enc['x_pl'].shape
# torch.Size([66, 50, 128])
# (Pdb) scene_enc['x_a'].shape
# torch.Size([62, 50, 128])
    def training_step(self,
        ...
        pred = self(data)
        if 'valid_mask_o' in data['agent']:
            reg_mask = data['agent']['valid_mask_o'][:, :self.num_historical_steps]
            cls_mask = data['agent']['valid_mask_o'][:, -1]
        else:
