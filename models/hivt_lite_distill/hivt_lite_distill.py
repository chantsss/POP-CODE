...
import random
import copyfrom models.common.utils import drop_hisfrom models.hivt_lite import HiVTLite

class HiVTLiteDistill(pl.LightningModule):
    def __init__(
        self,
        ...
        super(HiVTLiteDistill, self).__init__()
        ...
        if teacher_ckpt_path != '':
            ...
            self.teacher = HiVTLite.load_from_checkpoint(checkpoint_path=teacher_ckpt_path)
        else:
    ...
    def forward(self, data: TemporalData):
        ...
        # for x in [y_hat, pi]:
        #     print('sum is', x.sum())
        return y_hat, pi, teacher_local_embed, teacher_global_embed, local_embed, global_embed

    def training_step(self, data, batch_idx):
        # laplace nll loss
        y_hat, pi, teacher_local_embed, teacher_global_embed, local_embed, global_embed = self(data)
        ...
        loss = loss + self.distill_weight * feature_loss
        ...
        self.log(
            "train/feature_loss_local", feature_loss_local.item(), on_step=True, on_epoch=True, batch_size=1,sync_dist=True
        )
        self.log(
            "train/feature_loss_global", feature_loss_global.item(), on_step=True, on_epoch=True, batch_size=1,sync_dist=True
        )
        self.log(
            "train/feature_loss", feature_loss.item(), on_step=True, on_epoch=True, batch_size=1,sync_dist=True
        )
        return loss
    ...
    def validation_step(self, data, batch_idx):
        ...
        if self.use_multipath_loss:
            ...
            self.log("val/feature_loss_local", feature_loss_local, on_step=False, on_epoch=True, batch_size=1, sync_dist=True)
            self.log("val/feature_loss_global", feature_loss_global, on_step=False, on_epoch=True, batch_size=1, sync_dist=True)
            self.log("val/feature_loss", feature_loss, on_step=False, on_epoch=True, batch_size=1, sync_dist=True)
