...
import torch
import torch.nn as nnfrom utils.utils_hivt import TemporalDatafrom models.common.utils import init_weightsfrom models.hivt_plus.embedding import MultipleInputEmbedding, SingleInputEmbeddingfrom torch_geometric.nn.conv import MessagePassingfrom torch_geometric.typing import Adj, OptTensor, Sizefrom torch_geometric.utils import softmax, subgraph
...
class GlobalInteractor(nn.Module):
    ...
    def forward(
        self, data: TemporalData, local_embed: torch.Tensor, offset=0
    ) -> torch.Tensor:
        # * kinda of wired here, need padding mask here?
        edge_index, _ = subgraph(
            subset=~data["padding_mask"][:, self.historical_steps - 1 - offset],
            edge_index=data.edge_index,
        )
        rel_pos = (
            data["positions"][edge_index[0], self.historical_steps - 1 - offset]
            - data["positions"][edge_index[1], self.historical_steps - 1 - offset]
        )
