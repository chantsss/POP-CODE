...
import torch.nn.functional as Ffrom utils.utils_hivt import TemporalDatafrom models.common.utils import init_weightsfrom models.hivt_plus.embedding import MultipleInputEmbedding, SingleInputEmbeddingfrom models.hivt_plus.utils import DistanceDropEdgefrom torch_geometric.data import Batch, Datafrom torch_geometric.nn.conv import MessagePassingfrom torch_geometric.typing import Adj, OptTensor, Sizefrom torch_geometric.utils import softmax, subgraph
...
class LocalEncoder(nn.Module):
    def __init__(
        ...
        self.temporal_encoder = TemporalConsistentEncoder(
            historical_steps=historical_steps,
            embed_dim=embed_dim,
            num_heads=num_heads,
            dropout=dropout,
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        ...
        out1 = self.al_encoder(
            x=(data["lane_vectors"], out1),
            edge_index=edge_index,
            edge_attr=edge_attr,
            is_intersections=data["is_intersections"],
            turn_directions=data["turn_directions"],
...
class AAEncoder(MessagePassing):
    ...
    def forward(
        ...
        if self.parallel:
            ...
            if rotate_mat is None:
                ...
                # [nodes, 2] -> [nodes, 1, 2,] * [] -> [nodes, 2, 2] -> [nodes, 2]
                center_embed = self.center_embed(
                    torch.bmm(x.unsqueeze(-2), rotate_mat).squeeze(-2)
                )
            """
            bos_mask: [nodes]
            center_embed: [nodes, embed_dim]
            """
            center_embed = torch.where(
    ...
    def message(
        ...
        # [num_edges, embed_dim] -> [num_edges, head, head_dim]
        ...
        alpha = softmax(alpha, index, ptr, size_i)  # [edges]
        alpha = self.attn_drop(alpha)
        return value * alpha.unsqueeze(-1)  # [edges]
...
class TemporalConsistentEncoder(nn.Module):
    def __init__(
        self,
        historical_steps: int,
        embed_dim: int,
        ...
        super(TemporalConsistentEncoder, self).__init__()
        ...
        self.pos_embed = nn.Parameter(torch.Tensor(historical_steps + 2, 1, embed_dim))
        # attn_mask = self.generate_square_subsequent_mask(historical_steps + 1)
        attn_mask = self.generate_consistency_mask(historical_steps + 2)
    ...
    def forward(self, x: torch.Tensor, padding_mask: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.cat((x, expand_cls_token, expand_cls_token), dim=0)  # [T+2, N, D]
        x = x + self.pos_embed  # todo: need further consideration here
        out = self.transformer_encoder(
            src=x, mask=self.attn_mask, src_key_padding_mask=None
        )
        return out[-2], out[-1]  # [N, D]
    ...
    @staticmethod
    def generate_consistency_mask(seq_len: int) -> torch.Tensor:
