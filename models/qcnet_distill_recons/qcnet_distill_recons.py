...
from losses import MixtureNLLLossfrom losses import NLLLossfrom metrics.qc_metrics import Brierfrom metrics.qc_metrics import MRfrom metrics.qc_metrics import minADEfrom metrics.qc_metrics import minAHEfrom metrics.qc_metrics import minFDEfrom metrics.qc_metrics import minFHEfrom models.qcnet.qcnet_decoder import QCNetDecoderfrom models.qcnet_distill.qcnet_encoder import QCNetEncoder as QCNetEncoderDistillfrom models.qcnet_recons.qcnet_encoder import QCNetEncoder as QCNetEncoderReconsfrom models.qcnet.qcnet import QCNetfrom models.qcnet_distill.qcnet_distill import QCNetDistill
...
class QCNetDistill(pl.LightningModule):
    ...
    def __init__(self,
                ...
                recons_ckpt_path: str = '',
                **kwargs) -> None:
        super(QCNetDistill, self).__init__()
        ...
        self.distill_encoder = QCNetEncoderDistill(
            dataset=dataset,
            input_dim=input_dim,
        ...
        self.recons_encoder = QCNetEncoderRecons(
            dataset=dataset,
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            num_historical_steps=num_historical_steps,
        ...
        if distill_ckpt_path != '':
            print('loading teacher checkpoint...')
            distill_encoder_state_dict_pre = QCNet.load_from_checkpoint(checkpoint_path=distill_ckpt_path).encoder.state_dict()
            distill_encoder_state_dict = self.distill_encoder.state_dict()
            for name, param in distill_encoder_state_dict.items():
                if name in distill_encoder_state_dict_pre:
                    param.copy_(self.distill_encoder_state_dict_pre[name])
        else:
            print('Please input teacher checkpoint...')

        if recons_ckpt_path != '':
            print('loading reconstruction checkpoint...')
            recons_encoder_state_dict_pre = QCNet.load_from_checkpoint(checkpoint_path=recons_ckpt_path).encoder.state_dict()
            recons_encoder_state_dict = self.recons_encoder.state_dict()
            for name, param in recons_encoder_state_dict.items():
                if name in recons_encoder_state_dict_pre:
                    param.copy_(self.recons_encoder_state_dict_pre[name])
        else:
            print('Please input reconstruction checkpoint...')
    ...
    def forward(self, data: HeteroData):
        ...
        # teacher_pred = self.teacher.decoder(data)
        ...
        with torch.no_grad():
            distill_scene_enc = self.disll_encoder(data)
            recons_scene_enc = self.recons_ecoder(data)

        scene_enc = {}
        for feature in distill_scene_enc.keys():
            scene_enc[feature] = torch.cat(distill_scene_enc[feature], recons_scene_enc[feature], -1)
    ...
    def training_step(self,
        ...
        pred, scene_enc, teacher_scene_enc = self(data)
        ...
        feature_loss = 0
        for feature in teacher_scene_enc.keys():
            feature_loss += nn.MSELoss()(scene_enc[feature], teacher_scene_enc[feature].detach())

        # loss = nn.KLDivLoss()(nn.functional.log_softmax(student_outputs, dim=1),
        #                  nn.functional.softmax(teacher_outputs / 20, dim=1))
        ...
        self.log('feature_loss', feature_loss, prog_bar=False, on_step=True, on_epoch=True, batch_size=1)
        loss = reg_loss_propose + reg_loss_refine + cls_loss + feature_loss
