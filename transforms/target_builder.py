...
class TargetBuilder(BaseTransform):
    ...
    def __call__(self, data: HeteroData) -> HeteroData:
        ...
        data['agent']['target_recons'] = origin.new_zeros(data['agent']['num_nodes'], self.num_historical_steps, 4)
        data['agent']['target_recons'][..., :2] = torch.bmm(data['agent']['position'][:, :self.num_historical_steps, :2] -
                                                     origin[:, :2].unsqueeze(1), rot_mat)
        if data['agent']['position'].size(2) == 3:
            data['agent']['target_recons'][..., 2] = (data['agent']['position'][:, :self.num_historical_steps, 2] -
                                               origin[:, 2].unsqueeze(-1))
        data['agent']['target_recons'][..., 3] = wrap_angle(data['agent']['heading'][:, :self.num_historical_steps] -
                                                     theta.unsqueeze(-1))
