...
class AnalysisMetric:
    ...
    def save(self, path):
        torch.save(
            {
                "min_ade"+self.name: torch.cat(self.min_ade, dim=0),
                "min_fde"+self.name: torch.cat(self.min_fde, dim=0),
                "b_min_fde"+self.name: torch.cat(self.b_min_fde, dim=0),
                "seq_idx"+self.name: torch.cat(self.seq_idx, dim=0),
            },
            path,
        )
